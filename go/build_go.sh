...
check_prerequisites() {
    ...
    if [ ! -f "go.mod" ]; then
        log_error "go.mod not found. Run this script from the 'go' directory of the Mangatan project."
        exit 1
    fi
}
...
# Main
main() {
    log_info "Starting Go build for Mangatan"
